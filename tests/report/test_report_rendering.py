...
Coverage focus: report report rendering regression behavior.
"""
...
class ReportRenderingTests(unittest.TestCase):
    ...
    def test_report_sort_host_groups_findings_under_hosts(self):
        """Protect report sort host groups findings under hosts behavior from regressions."""
    ...
    def test_report_summary_table_fits_terminal_width(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            output = io.StringIO()
            with (
                patch("bywaf.runtime.table_widths.shutil.get_terminal_size", return_value=os.terminal_size((72, 24))),
    ...
    def test_report_prints_inline_by_default_and_can_page(self):
        """Protect report prints inline by default and can page behavior from regressions."""
        with tempfile.TemporaryDirectory() as tmp:
            runner = make_runner(Path(tmp, "bywaf.sqlite3"))
            runner.db.publish(
                "finding.candidate",
