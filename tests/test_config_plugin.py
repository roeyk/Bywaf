...
class ConfigPluginTests(unittest.TestCase):
    ...
    def test_scoped_varstore_reads_ancestor_provider_scope_explicitly(self):
        store = VarStore()
        store.set("cloud/aws.region", "us-east-1")
        store.set("cloud/aws/s3.bucket-list", "common.txt")
        context = CommandContext(
            db=None,
            source="s3_public_bucket_check",
            _varstore=store,
            metadata={
                "var_scope": "cloud/aws/s3/public_bucket/check",
                "provider_scope": "cloud/aws/s3/public_bucket",
            },
        )
        self.assertIsNone(context.vars.get_provider("region"))
        self.assertEqual(context.vars.get_provider_at("cloud/aws", "region"), "us-east-1")
        self.assertEqual(context.vars.get_provider_at("cloud/aws/s3", "bucket-list"), "common.txt")

    def test_effective_run_vars_include_provider_ancestors(self):
        store = VarStore()
        store.set("cloud/aws.region", "us-east-1")
        store.set("cloud/aws/s3.bucket-list", "common.txt")
        ...
        self.assertEqual(snapshot["cloud/aws.region"], "us-east-1")
        self.assertEqual(snapshot["cloud/aws/s3.bucket-list"], "common.txt")
