...
class StorageRunnerAuditDbTests(unittest.TestCase):
    ...
    def test_audit_list_policy_reports_no_matches(self):
        """Protect audit list policy reports no matches behavior from regressions."""
        with tempfile.TemporaryDirectory() as tmp:
            runner = make_runner(Path(tmp, "db.sqlite3"))
            output = io.StringIO()
            with contextlib.redirect_stdout(output):
    ...
    def test_audit_list_topics_prints_topic_policy_decisions(self):
        """Protect audit list topics prints topic policy decisions behavior from regressions."""
        with tempfile.TemporaryDirectory() as tmp:
            runner = make_runner(Path(tmp, "db.sqlite3"))
