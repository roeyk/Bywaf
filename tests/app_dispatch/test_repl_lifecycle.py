...
class AppDispatchTests(unittest.TestCase):
    ...
    def test_repl_confirms_keyboard_interrupt_before_exit(self):
        """Protect REPL confirms keyboard interrupt before exit behavior from regressions."""
        with tempfile.TemporaryDirectory() as tmp:
            runner = make_runner(Path(tmp, "db.sqlite3"))
    ...
    def test_repl_exits_after_confirmed_keyboard_interrupt(self):
        """Protect REPL exits after confirmed keyboard interrupt behavior from regressions."""
        with tempfile.TemporaryDirectory() as tmp:
            runner = make_runner(Path(tmp, "db.sqlite3"))
    ...
    def test_repl_exits_after_confirmed_eof(self):
        """Protect REPL exits after confirmed eof behavior from regressions."""
        with tempfile.TemporaryDirectory() as tmp:
            runner = make_runner(Path(tmp, "db.sqlite3"))
