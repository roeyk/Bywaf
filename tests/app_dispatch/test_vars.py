...
Coverage focus: app dispatch vars regression behavior.
...
class AppDispatchTests(unittest.TestCase):
    ...
    def test_vars_empty_explicit_secret_prompts_and_redacts(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("bywaf.repl.command.vars.load_fingerprint_key", return_value=b"k" * 32),
                patch("bywaf.repl.command.var.secrets.getpass.getpass", return_value="prompted-secret") as getpass,
                contextlib.redirect_stdout(output),
            ):
    ...
    def test_vars_empty_explicit_secret_uses_askpass_and_redacts(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("bywaf.repl.command.vars.load_fingerprint_key", return_value=b"k" * 32),
                patch("bywaf.repl.command.var.secrets.read_askpass_secret", return_value="askpass-secret") as askpass,
                contextlib.redirect_stdout(output),
            ):
                dispatch_repl_line(runner, "set --secret pw=", state)
                dispatch_repl_line(runner, "set pw", state)
    ...
    def test_vars_askpass_failure_warns_and_falls_back_to_getpass(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("bywaf.repl.command.vars.load_fingerprint_key", return_value=b"k" * 32),
                patch("bywaf.repl.command.var.secrets.read_askpass_secret", side_effect=AskpassUnavailable("no gui")),
                patch("bywaf.repl.command.var.secrets.getpass.getpass", return_value="fallback-secret") as getpass,
                contextlib.redirect_stdout(output),
    ...
    def test_vars_unknown_secret_input_mode_warns_and_falls_back(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("bywaf.repl.command.vars.load_fingerprint_key", return_value=b"k" * 32),
                patch("bywaf.repl.command.var.secrets.effective_secret_input_mode", return_value="block"),
                patch("bywaf.repl.command.var.secrets.getpass.getpass", return_value="fallback-secret") as getpass,
                contextlib.redirect_stdout(output),
                contextlib.redirect_stderr(errors),
            ):
    ...
    def test_vars_redacted_block_uses_hidden_secret_value(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch("bywaf.repl.command.vars.load_fingerprint_key", return_value=b"k" * 32),
                patch("bywaf.repl.command.var.secrets.getpass.getpass") as getpass,
                contextlib.redirect_stdout(output),
            ):
