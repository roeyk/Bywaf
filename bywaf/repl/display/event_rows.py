"""Compact one-line event row formatters.

Used by: `repl.display.events.print_events()` and app-level helpers that expose
`format_event()` for user-facing event listings.
"""
...
# payloads. format_event() checks this dispatch table first before falling back
# to simple infrastructure and prefix-based formatters.
