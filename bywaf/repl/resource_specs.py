"""Load/save resource spec parsing and path resolution.

Provides default resource paths, parsing for `load` and `save` specs, and path
resolution rules that preserve project defaults and explicit paths.

Used by:
- resource facade: route `load` and `save` commands to concrete handlers.
