"""Framework-mediated subprocess API exposed through command contexts.

Used by:
- `CommandContext.process`: gives commandlets an audited subprocess runner.
- process-wrapped plugins: run external tools without importing `subprocess`
  directly or bypassing framework capability/audit handling.
"""
...
from .exec import popen_process_argv
...
def public_run_process_argv():
    """Return the package-level process runner patch seam.

    Called by: `ContextProcess.run()` so tests and integrations that patch
    `bywaf.plugin.process.run_process_argv` still intercept framework-mediated
    process execution after `ContextProcess` moved out of the package
    initializer.
    """
    return sys.modules["bywaf.plugin.process"].run_process_argv
...
class ContextProcess:
    ...
    def run(
        ...
        *,
        ...
        completed = public_run_process_argv()(normalized, cwd=payload["cwd"], env=env, timeout=timeout)
