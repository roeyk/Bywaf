"""Prompt-toolkit editing helpers for redacted secret blocks.

Used by:
- `bywaf.secret.input`: re-exports the prompt helpers as part of the public
  secret-input API.
- completion prompt setup: installs key bindings, lexer styling, and an output
  proxy so plaintext secrets stay outside the visible command buffer.
"""
...
from .state import PromptSecretInputState
