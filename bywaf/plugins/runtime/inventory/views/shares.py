...
def render_shares_inventory(context: CommandContext, events: list[Event], scope: str, sort: str = "host") -> str:
    ...
    Called by: runtime inventory commandlets for the `shares` view.
    """
    ...
    # Authenticated is tri-state in source payloads; keep unknown values blank
    # so the table does not imply a false negative.
    rows = [
        (
...
def share_sort_key(event: Event, key: str) -> Any:
    """Return a sortable share event value.

    Called by: `render_shares_inventory()`.
    """
